...
def intrin_4x8_gemm_neon_ir():
    ...
    z = clang.create_llvm(src, options=["-O3", "--target=armv7-none-linux-gnueabihf", "-mcpu=cortex-a53"], output="/tmp/x.ll")
    return "/tmp/x.ll"
...
def conv2d_NCHWc_direct_autotvm(s, ic, oc, kernel, pad, stride):
    ...
    cfg.define_knob('BNInput', [8]) # TODO, 8, 16
    ...
    W = tvm.placeholder(shape=(oc // BNOutput, kernel, kernel, ic, BNOutput), dtype="int8", name="W")
