...
TVM_REGISTER_GLOBAL("tvm.contrib.xnnpack.conv1d")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      auto setup = [&]() {
        ...
        CHECK_EQ(
            xnn_create_convolution2d_nhwc_f32(
                0, padding_width, 0, padding_width, 1, kernel_width, 1,
                stride_width, 1, dilation_width, 1, input_channels,
                output_channels, input_channels, output_channels,
                (const float*)(kernel->data), nullptr,
                std::numeric_limits<float>::lowest(),
                std::numeric_limits<float>::max(), XNN_FLAG_INPUT_NHWC, &op_t),
            xnn_status_success);
        CHECK(op_t != nullptr);
      ...
      const auto key = std::make_tuple(
          kernel->data, padding_width, kernel_width, stride_width,
          dilation_width, input_channels, output_channels);

      if (opcache.find(key) == opcache.end()) {
        LOG(INFO) << "Creating op";
        opcache.emplace(key, setup());
      }
      CHECK(opcache.find(key) != opcache.end());
...
TVM_REGISTER_GLOBAL("tvm.contrib.xnnpack.conv1d_transpose")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      auto setup = [&]() {
        ...
        CHECK_EQ(
            xnn_create_deconvolution2d_nhwc_f32(
                0, padding_width, 0, padding_width, 1, kernel_width, 1,
                stride_width, 1, 1, 1, input_channels, output_channels,
                input_channels, output_channels, (const float*)(kernel->data),
                nullptr, std::numeric_limits<float>::lowest(),
                std::numeric_limits<float>::max(), XNN_FLAG_INPUT_NHWC, &op_t),
            xnn_status_success);
        CHECK(op_t != nullptr);
      ...
      const auto key = std::make_tuple(
          kernel->data, padding_width, kernel_width, stride_width,
          output_adjustment_width, input_channels, output_channels);
