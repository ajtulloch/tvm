...
#include "vulkan2_module.h"
#include "vulkan2_stream.h"
...
class Vulkan2ThreadEntry {
 public:
  Vulkan2ThreadEntry();
  static Vulkan2ThreadEntry* ThreadLocal();
  ...
  TVMContext ctx;
  WorkspacePool pool;
  Vulkan2Stream* Stream(size_t device_id);
  Vulkan2StagingBuffer* StagingBuffer(int device_id, size_t size);

 private:
  std::unordered_map<size_t, std::unique_ptr<Vulkan2Stream>> streams_;
  std::unordered_map<size_t, std::unique_ptr<Vulkan2StagingBuffer>> staging_buffers_;
};

struct Vulkan2Buffer {
  VkBuffer buffer{VK_NULL_HANDLE};
  VkDeviceMemory memory{VK_NULL_HANDLE};
};

struct Vulkan2Pipeline {
  Vulkan2Context* vctx_{nullptr};
...
typedef dmlc::ThreadLocalStore<Vulkan2ThreadEntry> Vulkan2ThreadStore;

class Vulkan2DeviceAPI final : public DeviceAPI {
 public:
  Vulkan2DeviceAPI();
  ~Vulkan2DeviceAPI() {
  ...
  void SetDevice(TVMContext ctx) final { Vulkan2ThreadEntry::ThreadLocal()->ctx = ctx; }
  ...
  void* AllocDataSpace(TVMContext ctx, size_t nbytes, size_t alignment, TVMType type_hint) final {
    ...
    Vulkan2Buffer* pbuf = new Vulkan2Buffer();
    pbuf->memory = memory;
    pbuf->buffer = buffer;
    return pbuf;
  }
  ...
  void FreeDataSpace(TVMContext ctx, void* ptr) final {
    const auto& vctx = context(ctx.device_id);
    auto* pbuf = static_cast<Vulkan2Buffer*>(ptr);
  ...
  void CopyDataFromTo(const void* from, size_t from_offset, void* to, size_t to_offset, size_t size,
    ...
    if (from_dev_type == kDLVulkan && to_dev_type == kDLVulkan) {
      Vulkan2ThreadEntry::ThreadLocal()
          ->Stream(ctx_from.device_id)
          ->Launch([=](Vulkan2StreamState* state) {
            // 1: copy
            const auto* from_buf = static_cast<const Vulkan2Buffer*>(from);
            auto* to_buf = static_cast<Vulkan2Buffer*>(to);
      ...
      auto* temp = Vulkan2ThreadEntry::ThreadLocal()->StagingBuffer(ctx_from.device_id, size);
      Vulkan2ThreadEntry::ThreadLocal()
          ->Stream(ctx_from.device_id)
          ->Launch([&](Vulkan2StreamState* state) {
            VkBufferCopy copy_info;
      ...
      Vulkan2ThreadEntry::ThreadLocal()->Stream(ctx_from.device_id)->Synchronize();
      ...
      const auto* to_buf = static_cast<const Vulkan2Buffer*>(to);
      Vulkan2StagingBuffer* temp =
          Vulkan2ThreadEntry::ThreadLocal()->StagingBuffer(ctx_to.device_id, size);
      ...
      // TODO: should we instead make the staging buffer a property of the
  ...
  void* AllocWorkspace(TVMContext ctx, size_t size, TVMType type_hint) final {
    return Vulkan2ThreadEntry::ThreadLocal()->pool.AllocWorkspace(ctx, size);
  }

  void FreeWorkspace(TVMContext ctx, void* data) final {
    Vulkan2ThreadEntry::ThreadLocal()->pool.FreeWorkspace(ctx, data);
  }

  static const std::shared_ptr<Vulkan2DeviceAPI>& Global() {
    static std::shared_ptr<Vulkan2DeviceAPI> inst = std::make_shared<Vulkan2DeviceAPI>();
    return inst;
  }

  const Vulkan2Context& context(size_t device_id) const {
 ...
 private:
  ...
  std::vector<Vulkan2Context> context_;
};

void Vulkan2DeviceAPI::GetAttr(TVMContext ctx, DeviceAttrKind kind, TVMRetValue* rv) {
...
Vulkan2DeviceAPI::Vulkan2DeviceAPI() {
  ...
  for (VkPhysicalDevice phy_dev : all_phy_devs) {
    ...
    const auto extensions = [&]() {
      ...
      for (const auto& dp : device_extension_prop) {
        ...
        if ((std::strcmp(dp.extensionName, "VK_KHR_dedicated_allocation") == 0) && dp.specVersion > 0) {
...
#define USE_VULKAN_IMMEDIATE 1
#ifdef USE_VULKAN_IMMEDIATE
    if (has_extension("VK_KHR_push_descriptor") &&
      ...
      ctx.descriptor_template_khr_functions =
          std::unique_ptr<Vulkan2DescriptorTemplateKHRFunctions>(
              new Vulkan2DescriptorTemplateKHRFunctions());
...
#define USE_VULKAN_DEDICATED_ALLOCATION 1
    ...
    if (has_extension("VK_KHR_get_memory_requirements2") &&
      ...
      ctx.get_buffer_memory_requirements_2_functions =
          std::unique_ptr<Vulkan2GetBufferMemoryRequirements2Functions>(
              new Vulkan2GetBufferMemoryRequirements2Functions());
...
class Vulkan2ModuleNode;

// a wrapped function class to get packed func.
class Vulkan2WrappedFunc {
 public:
  void Init(Vulkan2ModuleNode* m, std::shared_ptr<ModuleNode> sptr, const std::string& func_name,
 ...
 private:
  ...
  mutable std::array<std::shared_ptr<Vulkan2Pipeline>, kVulkanMaxNumDevice> scache_;
};

// Multi-device enabled module.
class Vulkan2ModuleNode final : public runtime::ModuleNode {
 public:
  explicit Vulkan2ModuleNode(std::unordered_map<std::string, VulkanShader> smap,
  ...
  ~Vulkan2ModuleNode() {
    ...
    for (int device_id = 0; device_id < ecache_.size(); ++device_id) {
      for (auto& kv : ecache_[device_id]) {
        auto& pe = kv.second;
        CHECK(pe);
        const auto& vctx = Vulkan2DeviceAPI::Global()->context(device_id);
  ...
  std::shared_ptr<Vulkan2Pipeline> GetPipeline(size_t device_id, const std::string& func_name,
    ...
    // Create new pipeline
    auto pe = std::shared_ptr<Vulkan2Pipeline>(new Vulkan2Pipeline());
    {
      // create shader
 ...
 private:
  ...
  // TODO: thread safety?
  std::mutex mutex_;
  std::array<std::unordered_map<std::string, std::shared_ptr<Vulkan2Pipeline>>, kVulkanMaxNumDevice>
      ecache_;
};

Module VulkanModuleCreate(std::unordered_map<std::string, VulkanShader> smap,
                          std::unordered_map<std::string, FunctionInfo> fmap, std::string source) {
  std::shared_ptr<Vulkan2ModuleNode> n = std::make_shared<Vulkan2ModuleNode>(smap, fmap, source);
  return Module(n);
}

Vulkan2ThreadEntry* Vulkan2ThreadEntry::ThreadLocal() { return Vulkan2ThreadStore::Get(); }

Vulkan2StagingBuffer* Vulkan2ThreadEntry::StagingBuffer(int device_id, size_t size) {
  if (!staging_buffers_[device_id]) {
    staging_buffers_[device_id] = std::unique_ptr<Vulkan2StagingBuffer>(new Vulkan2StagingBuffer());
  }
...
Vulkan2ThreadEntry::Vulkan2ThreadEntry()
    : pool(static_cast<DLDeviceType>(kDLVulkan), Vulkan2DeviceAPI::Global()) {
  ctx.device_id = 0;
  ctx.device_type = static_cast<DLDeviceType>(kDLVulkan);
}

Vulkan2Stream* Vulkan2ThreadEntry::Stream(size_t device_id) {
  if (!streams_[device_id]) {
    streams_[device_id] = std::unique_ptr<Vulkan2Stream>(
        new Vulkan2Stream(&Vulkan2DeviceAPI::Global()->context(device_id)));
  }
  return streams_[device_id].get();
};

void Vulkan2WrappedFunc::operator()(TVMArgs args, TVMRetValue* rv,
                                    const ArgUnion* pack_args) const {
  int device_id = Vulkan2ThreadEntry::ThreadLocal()->ctx.device_id;
  ...
  for (int i = 0; i < num_buffer_args_; ++i) {
    ...
    binfo.buffer = static_cast<Vulkan2Buffer*>(buf)->buffer;
    binfo.offset = 0;
  ...
  if (vctx.UseImmediate()) {
    ...
    Vulkan2ThreadEntry::ThreadLocal()->Stream(device_id)->Launch([&](Vulkan2StreamState* state) {
  ...
  const auto& deferred_kernel = [pipeline, wl, pack_args_storage](Vulkan2StreamState* state) {
    vkCmdBindPipeline(state->cmd_buffer_, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline->pipeline);
  ...
  Vulkan2StreamToken deferred_token;
  ...
  Vulkan2ThreadEntry::ThreadLocal()->Stream(device_id)->LaunchDeferred(
...
TVM_REGISTER_GLOBAL("device_api.vulkan").set_body([](TVMArgs args, TVMRetValue* rv) {
  DeviceAPI* ptr = Vulkan2DeviceAPI::Global().get();
