...
class Vulkan2Stream {
 public:
  Vulkan2Stream(const Vulkan2Context* vctx) : vctx_(vctx), state_(new Vulkan2StreamState()) {
    // create command pool
  ...
  void Launch(const std::function<void(Vulkan2StreamState*)>& kernel) {
  ...
  void LaunchDeferred(const std::function<void()>& deferred_initializer,
                      const std::function<void(Vulkan2StreamState*)>& deferred_kernel,
                      const Vulkan2StreamToken& deferred_token) {
    ...
    if (std::any_of(deferred_tokens_.begin(), deferred_tokens_.end(),
                    [&](const Vulkan2StreamToken& token) {
  ...
  void Synchronize() {
    ...
    {
      // Multiple streams (on different threads) use the same Vulkan2Context
 ...
 private:
  const Vulkan2Context* vctx_;
  std::unique_ptr<Vulkan2StreamState> state_;
  std::vector<Vulkan2StreamToken> deferred_tokens_;
  std::vector<std::function<void(Vulkan2StreamState*)>> deferred_kernels_;
