...
def decl_packed_winograd(cfg, input_shape, packed_input, kernel, strides, padding, layout, out_dtype):
    CO, _, KH, KW = get_const_tuple(kernel.shape)
    HSTR, WSTR = strides if isinstance(strides, (tuple, list)) else (strides, strides)
    HPAD, WPAD, _, _ = get_pad_tuple(padding, kernel)
    assert KH == 3 and KW == 3 and HPAD == 1 and WPAD == 1 and HSTR == 1 and WSTR == 1
    ...
    r = 3
    alpha = m + r - 1
    (N, CI, H, W) = input_shape
    (K_VK, P_VP, m, m, VK, VP) = get_const_tuple(packed_input.shape)
    K = K_VK * VK
    # unpack output
    nH, nW = (H + m-1) // m, (W + m-1) // m
    ...
    def _unpacked_input(n, k_, h, w):
        b_idx = n * nH * nW + (h//m) * nW + w//m
        b = b_idx // VP
        bb = b_idx % VP
        k = k_ // VK
        kk = k_ % VK
        return packed_input[k][b][h % m][w % m][kk][bb]

    HPAD = WPAD = 1
    unpacked_input = tvm.compute(
        (N, K, H, W), _unpacked_input,
        name='output', tag='winograd_unpack_input')
    data_pad = pad(unpacked_input, (0, 0, HPAD, WPAD), name="data_pad")

    input_tile = tvm.compute((P // VP, K, alpha, alpha, VP),
                             lambda b, c, eps, nu, bb:
                             data_pad[(b*VP+bb) // (nH*nW)][c][(b*VP+bb) // nW % nH * m + eps]
                             [(b*VP+bb) % nW * m + nu] + tvm.const(0, data_pad.dtype) * data_pad[d0 - 1, d1 - 1, d2 - 1, d3 - 1],
                             name='input_tile')
    ...
    packed_output = tvm.compute((K // VK, P // VP, m, m, VK, VP), compute_X_dot_A, name="Y")
...
def schedule_winograd(cfg, output, VK, VP):
    ...
    if cfg['input_tile_cache_reorder'].val:
        s[input_tile_cache].reorder(b, c, eps, bb, nu)

    if cfg['input_tile_cache_REORDER_C'].val:
        s[input_tile_cache].reorder(b, c, eps, bb, nu)
    # s[input_tile_cache].unroll(nu)
