from ..nn import upsampling_alter_layout

@upsampling_alter_layout.register(["arm_cpu"])
def _alter_upsampling_layout_arm(attrs, inputs, tinfos, F):
    ...
    (data, ) = tinfos
    if data.dtype == "int8" or data.dtype == "int32":
        new_attrs[data_layout_key] = 'NHWC'
        return F.nn.upsampling(*copy_inputs, **new_attrs)
