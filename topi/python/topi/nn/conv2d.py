...
def conv2d(input, filter, strides, padding, dilation, layout='NCHW', out_dtype=None):
    ...
    if layout == 'NHWC' and len(filter.shape) == 4:
        return conv2d_nhwc(input, filter, strides, padding, dilation, out_dtype)
    if layout == 'NHWC' and len(filter.shape) == 5:
        return conv2d_nhwc_OHWIo(input, filter, strides, padding, dilation, out_dtype)
...
def conv2d_nhwc_OHWIo(Input, Filter, stride, padding, dilation, out_dtype='float32'):
    """Convolution operator in NHWC layout.
    ...
    coo, kernel_h, kernel_w, ci, vc = get_const_tuple(Filter.shape)
    # compute the output shape
    dilated_kernel_h = (kernel_h - 1) * dilation_h + 1
    dilated_kernel_w = (kernel_w - 1) * dilation_w + 1
    pad_top, pad_left, pad_down, pad_right = get_pad_tuple(
    ...
    Output = tvm.compute(
        ...
        lambda nn, yy, xx, ff: tvm.sum(
            PaddedInput[nn, yy * stride_h + ry * dilation_h,
                        xx * stride_w + rx * dilation_w, rc].astype(out_dtype) *
            Filter[rc // vc, ry, rx, rc, ff % vc].astype(out_dtype), axis=[ry, rx, rc]),
        name="Conv2dOutput", tag="conv2d_nhwc")
    return Output
