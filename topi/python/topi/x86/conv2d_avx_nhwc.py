...
def conv2d_nhwc_tensor_1x1(A, W_, stride, padding):
    dtype = A.dtype
...
def schedule_conv2d_nhwc_tensor_1x1(outs):
...
def verify_conv2d_nhwc(batch, in_channel, in_size, num_filter, kernel, stride, padding, dilation=1):
    ...
    def check_device(device):
        ...
        evaluator_nchw = func_nchw.time_evaluator(func_nchw.entry_name, ctx, number=REPEAT)
        # evaluator_tensor = func_tensor.time_evaluator(func_tensor.entry_name, ctx, number=REPEAT)
        evaluator_tensor_mxn = func_tensor_mxn.time_evaluator(func_tensor_mxn.entry_name, ctx, number=REPEAT)
        print('Baseline: %f' % (FLOPS / evaluator(a, w, b).mean / 1E9))
        print('BaselineNCHW: %f' % (FLOPS / evaluator_nchw(a_nchw, w_nchw, b_nchw).mean / 1E9))
        # print('Tensor: %f' % (FLOPS / evaluator_tensor(a, w, b_tensor).mean / 1E9))
        print('TensorMxN: %f' % (FLOPS / evaluator_tensor_mxn(a, w, b_tensor_mxn).mean / 1E9))
