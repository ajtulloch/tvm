...
def verify_conv2d_NCHWc(batch, in_channel, in_size, num_filter, kernel, stride, padding, dilation=1):
    ...
    b_NCHWc_custom = tvm.nd.array(nchw_to_NCHWc(np.zeros(get_const_tuple(B_NCHW.shape), dtype=B_NCHW.dtype)), ctx)
    ...
    func_NCHWc_custom = remote_func(tvm.build(s_NCHWc_custom, [A_NCHWc, W_NCHWc, B_NCHWc_custom], target), name="func_NCHWc")
    ...
    func_NCHWc_custom(a_NCHWc, w_NCHWc, b_NCHWc_custom)
    np.testing.assert_allclose(NCHWc_to_nchw(b_NCHWc_custom.asnumpy()), b_np.transpose(0, 3, 1, 2), rtol=1e-5)
